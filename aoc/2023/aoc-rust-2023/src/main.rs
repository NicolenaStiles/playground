mod day_1;
mod day_2;
mod day_3;
mod day_4;
mod day_5;
mod day_7;

fn main() {

    day_1::part_one("input/day_1/data".to_string());
    day_1::part_two("input/day_1/data".to_string());

    day_2::part_one("input/day_2/data".to_string());
    day_2::part_two("input/day_2/data".to_string());

    //day_3::part_one_better("input/day_3/test_1".to_string());

    //day_4::part_one("input/day_4/data".to_string());
    //day_4::part_two("input/day_4/test_1".to_string());

    //day_5::part_one("input/day_5/test_1".to_string());
    day_7::part_one("input/day_7/test_1".to_string());
}
